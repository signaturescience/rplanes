...
prepped_forecast <-
  tibble(
    ...
    horizon = 1:4,
    lower = point_est - 10,
    ## make an normal, familiar shape in hospitalizations to not trigger shape component
    point = point_est,
    upper = point_est + 10,
  ) %>%
  to_signal(outcome = "flu.admits", type = "forecast", horizon = 4)
...
We can visualize this below. The signal in red (a forecast in this example) has a zero around July 1st (red diamond), but there were no zeros found in the seed data in blue.
...
zero_dat %>%
  ...
  geom_line(lty = "dotted") +
  geom_line(aes(colour = type)) +
    geom_point(data = zero_flags, mapping = aes(x = date, y = flu.admits), shape=23, size=4, color = "black", fill = "red") +
  geom_point(aes(colour = type)) +
  xlab("Date") +
  ylab("Flu hospitalizations") +
...
We can visualize this below. The signal in red (a forecast in this example) has a zero around early-mid June (diamond), but there were also zeros (diamonds) found in the seed data in blue.
