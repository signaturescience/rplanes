...
By toggling the significance level, we can control the sensitivity of the trend assessment. Here we lower the significance level to `0.001` and see that the flag is no longer raised using the same data as above in section **0.7.1**:
...
### Not flagged as implausible but does have change point in the observed data
There are some cases where `plane_trend()` will identify a change point in the observed data but will not raise an implausibility flag for the forecast. This only occurs when there is a significant change point found in the observed data prior to the last observed time stamp. We include this in the output tibble to provide more information to users. Here we will look at location "06" where that occurs. We need to define a new prepped_seed, because this changepoint occurs after our previously defined cut date:
```{r}
## create seed with cut date
...
```
Note that on 2022-09-17, a changepoint was observed, but because it was not in our forecast, nor was it the last observed data point, a flag was not raised.

The visualization below shows the changepoint in the observed data:
...
ggplot(data = trend_dat, mapping = aes(x = date, y = flu.admits)) +
  ...
  xlab("Date") +
  ylab("Flu hospitalizations") +
  ggtitle(paste("Trend component\nUnflagged Changepoint in Observed Signal"))
```
We can see the change point on 2022-09-17 (diamond), but it was not flagged, because it was not in the forecast or the last observed point.
