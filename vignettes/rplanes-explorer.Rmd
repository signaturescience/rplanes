...
The app is delivered as a function in the `rplanes` package. To launch the app, users can run the `rplanes_explorer()` function. Note that this function wraps `shiny::runApp()` and inherits any arguments. For example, adding an argument for `launch.browser = TRUE` will open up the app in a web browser and setting `port = 80` will run the app at port `80` in the localhost.
