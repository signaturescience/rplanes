...
It is important to emphasize that the observed data must at minimum include columns for location (geographic unit such as FIPS code) and date (date of reported value; must be date class).

#### Convert observed data into a signal with `to_signal()`

The downstream plausibility analysis requires that observed and/or forecasted data be converted to an S3 "signal" class. The `to_signal()` function constructs this object and applies an additional "observed" or "forecast" class depedning on the type of signal specified.
...
The forecast to evaluate must include a point estimate and some representation of uncertainty (i.e., prediction interval). For analysis in `rplanes`, the forecast must be prepared to include the following:
...
The `planes_score()` function wraps PLANES scoring for plausibility components across all locations in single step. The function can accept arguments to specific component functions and allows the user to dictate which components to use (with a default to all). For more about these arguments see `?plane_score`.

We pass the input forecast signal to the fucntion along with the prepped seed to run the scoring independently at all locations:
...
The output of `plane_score()` is a list of scoring results for all locations. We can convert the `scores_summary` element from that list to a tibble. The `res` tibble below shows location, the number of flags raised ("n_flags"), the number of components tested ("n_components"), the "score", which is "n_flags" divided by "n_components", the names of components tested, and the names of the components that were flagged (if any):
...
For the evaluated forecast, most of the locations had no flags raised. However, several had three of the five possible components flagged for implausibility.
...
One might want to test observed or reported data for plausibility to check for things like backfill or errors in reporting mechanisms. The steps to use `rplanes` to analyze plausibility of observed data are very similar to forecast evaluations. The input data must be converted to a signal, which is passed (along with a seed) to the scoring function. However, the key difference is that for observed data the same data source used to create the input signal can be used for seeding.
...
```{r}
scores <- plane_score(observed_signal, seed = prepped_seed, components = c("repeats","diff"))
```
