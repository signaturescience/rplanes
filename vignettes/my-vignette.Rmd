...
This function has two arguments and it wraps the seed engine to operate across all locations in the input signal:
...
We will repeat the same steps as above, but this time we're comparing observed data to the seed to check for plausibility. We will convert our results to a tibble as we did above:
```{r}
# Run plane_score:
...
* `prepend`: Integer value for the number of values from seed to add before the evaluated signal. Default is NULL and the number of values will be determined from seed also by using the maximum number of repeats. **LOGIC HERE NEEDS TO BE EXPLAINED A BIT**
...
**ADD PLOTS WITH BARS IN PLOT HELPERS**
...
```
The black horizontal line in the plot below is the tolerance length, and you can see that it is the same length as the repeats in the forecast; therefore, no flags have been raised:
```{r fig.align = 'center'}
ggplot(data = repeat_dat_flag, mapping = aes(x = date, y = flu.admits)) +
  geom_line() +
  geom_line(aes(colour = type)) +
...
Note that the opposite is also true. If you reduce the tolerance, you increase the sensitivity, and a flag may be raised when it wasn't otherwise:
```{r}
plane_repeat(location = "32", input = forecast_signal, seed = prepped_seed, tolerance = 3, prepend = NULL)
```
The black horizontal line in the plot below is the tolerance length, and you can see that it is shorter than the repeats in the forecast; therefore, a flag has been raised:
```{r fig.align = 'center'}
ggplot(data = repeat_dat_flag, mapping = aes(x = date, y = flu.admits)) +
  geom_line() +
  geom_line(aes(colour = type)) +
...
#### E. Sensitivity and the prepend argument:
```{r}
plane_repeat(location = "32", input = forecast_signal, seed = prepped_seed, tolerance = NULL, prepend = NULL)
```
 
#### F. An example where a flag IS NOT raised and the data is plausible when we change prepend default

#### H. An example where a flag IS raised and the data is implausible when we change prepend default:

#### I. An example using this function to look at plausibility of observed data:
